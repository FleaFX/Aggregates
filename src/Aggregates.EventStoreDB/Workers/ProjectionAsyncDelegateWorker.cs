...
class ProjectionAsyncDelegateWorker<TProjection, TEvent>(IServiceScopeFactory serviceScopeFactory, ILogger<ProjectionDelegateWorker<TProjection, TEvent>> logger)
                        ...
                        switch (message) {
                            case PersistentSubscriptionMessage.Event @event when !Equals(@event.ResolvedEvent.OriginalPosition, skipPosition): {
                                try {
                                    ...
                                    await commit.CommitAsync(stoppingToken);
    ...
    static async Task<(string? subscriptionGroupName, ProjectionAsyncDelegate<TEvent>? @delegate, IPosition? skipPosition)> BootstrapAsync(TProjection owner, ListToAllAsyncDelegate listToAllAsync, CreateToAllAsyncDelegate createToAllAsync, DeleteToAllAsyncDelegate deleteToAllAsync, AggregatesOptions options, CancellationToken cancellationToken) {
        ...
        var @delegate = (
                ...
                where method.IsDelegate<ProjectionAsyncDelegate<TEvent>>(owner)
                select method.CreateDelegate<ProjectionAsyncDelegate<TEvent>>(owner)
            ).SingleOrDefault();
        if (@delegate is null)
            return (null, null, null);
        ...
        var eventTypes = (
            ...
            select (type, attr)
        ).TrySelectAsync(async tuple => {
            var (eventType, contract) = tuple;
            await @delegate((TEvent)Activator.CreateInstance(eventType)!, cancellationToken: cancellationToken);
            return contract;
        });
