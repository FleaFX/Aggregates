...
public sealed record SagaRoot<TState, TEvent>(TState State, AggregateVersion Version) : IAggregateRoot where TState : IState<TState, TEvent> {
    ...
    /// <param name="delegate">A <see cref="SagaAsyncDelegate{TState, TReactionEvent,TCommand,TState,TEvent}"/> that implements the reaction logic.</param>
    ...
    public IAsyncEnumerable<TCommand> AcceptAsync<TCommand, TCommandState, TCommandEvent>(SagaAsyncDelegate<TState, TEvent, TCommand, TCommandState, TCommandEvent> @delegate, TEvent @event, IReadOnlyDictionary<string, object?> metadata, CancellationToken cancellationToken = default)
        where TCommand : ICommand<TCommandState, TCommandEvent>
        where TCommandState : IState<TCommandState, TCommandEvent> {
        _changes.Add(@event!);
        return @delegate(State, @event, metadata, cancellationToken);
    }
