...
interface IRepository<TState, TEvent> where TState : IState<TState, TEvent> {
    /// <summary>
    ...
    ValueTask<AggregateRoot<TState, TEvent>?> GetAsync(AggregateIdentifier identifier);
    ...
    void Add(AggregateIdentifier identifier, AggregateRoot<TState, TEvent> aggregateRoot);
}
...
abstract class BaseRepository<TState, TEvent> : IRepository<TState, TEvent> where TState : IState<TState, TEvent> {
    ...
    /// <param name="identifier">Uniquely identifies the aggregate to retrieve.</param>
    /// <returns>An awaitable <see cref="ValueTask{TResult}"/>, which resolves to a <see cref="AggregateRoot{TState,TEvent}"/>.</returns>
    public async ValueTask<AggregateRoot<TState, TEvent>?> GetAsync(AggregateIdentifier identifier) {
